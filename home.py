...
① 서비스 접근 및 이용 관리: 학급별 URL 파라미터를 통한 수강 대상 식별, 해당 교과 자료 제공 범위 제한  
② 수업 자료 및 학습 활동 제공: 교과별 강의 자료(슬라이드·PDF·영상) 및 인터랙티브 시뮬레이션 활동 제공  
③ 학습 활동 결과 기록 및 관리: 학생이 활동 후 성찰 기록 제출 시 입력한 학번·이름·활동 내용을 수집하여 구글 시트(Google Sheets)에 저장하며, 교사의 학습 확인 및 피드백 제공에 활용  
④ 교사의 진도 관리 및 상담 지원: 날짜·반별 수업 진도 기록, 구글 시트 연동을 통한 진도 데이터 관리, 체계적인 수업 운영 지원  
⑤ 관리자(교사) 인증: 관리자 비밀번호를 통한 접근 권한 확인, 진도표 관리 등 관리자 전용 기능 제공  
...
- **수집 항목**: 학번(학년·반·번호), 이름, 활동 내용(문제·답·소감)  
...
② **파기 방법**: 전자적 파일 형태로 기록·저장된 개인정보는 기록을 재생할 수 없도록 파기(영구 삭제)하며, 종이 문서는 분쇄하거나 소각하여 파기합니다.  
...
① **해킹 등에 대비한 기술적 대책**: 보안 인증을 획득한 전문 클라우드 플랫폼(Google Sheets, GitHub)을 기반으로 운영되며, 전 구간 보안 통신(HTTPS)을 사용하여 데이터를 암호화하여 전송합니다.  
