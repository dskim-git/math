...
① **회원 가입 및 인증**: 학생·일반인 회원가입 시 아이디·비밀번호·이름·학번(학생의 경우)을 수집하여 로그인 인증 및 서비스 접근 권한 관리에 활용합니다.  
② **서비스 접근 및 이용 관리**: 로그인 기반 사용자 인증을 통해 교과별 자료 제공 범위를 제한하고, 학생 회원에게 적합한 기능을 제공합니다.  
③ **수업 자료 및 학습 활동 제공**: 교과별 강의 자료(슬라이드·PDF·영상) 및 인터랙티브 시뮬레이션 활동을 제공합니다.  
④ **학습 활동 결과 기록 및 관리**: 학생이 활동 후 성찰 기록 제출 시 학번·이름·활동 내용을 수집하여 Google Sheets에 저장하며, 교사의 학습 확인 및 피드백 제공에 활용합니다. 학생은 자신의 과거 성찰 제출 이력을 서비스 내에서 직접 조회할 수 있습니다.  
⑤ **방문 기록 관리**: 서비스 개선을 위해 방문 시각·과목 필터·사용자 ID를 Google Sheets에 기록합니다. 해당 데이터는 관리자(교사)만 열람 가능합니다.  
⑥ **교사의 진도 관리 및 상담 지원**: 날짜·반별 수업 진도 기록, Google Sheets 연동을 통한 진도 데이터 관리로 체계적인 수업 운영을 지원합니다.  
⑦ **관리자(교사) 인증**: 관리자 비밀번호를 통한 접근 권한 확인, 회원 관리·진도표 관리 등 관리자 전용 기능을 제공합니다.  
...
| 구분 | 수집 항목 |
|---|---|
| 학생 회원 | 아이디, 비밀번호(암호화 저장), 이름, 학번(학년·반·번호), 학년 |
| 일반 회원 | 아이디, 비밀번호(암호화 저장), 이름, 사용목적, 소속그룹 |
| 성찰 기록 | 학번(뒷 5자리), 이름, 활동명, 제출 시각, 활동별 답변 내용 |
...
① **비밀번호 암호화**: 이용자의 비밀번호는 bcrypt 단방향 암호화(Hash)되어 저장·관리되며, 개발자(관리자)도 원문을 알 수 없습니다.  
...
② 학생 회원은 서비스 내 **[내 성찰 기록]** 메뉴를 통해 자신의 제출 이력을 직접 열람할 수 있습니다.  
③ 개인정보 삭제·탈퇴 요청은 개발 교사에게 구두나 서면으로 요청하면 지체 없이 조치하겠습니다.  
