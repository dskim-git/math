...
def render():
    ...
    with c1:
        st.metric("z(α/2)", f"{z:.3f}")  # metric에서는 LaTeX 미지원 → 텍스트로 표기
    with c2:
    ...
    fig1 = go.Figure()
    fig1.add_scatter(x=n_grid, y=L_n, mode="lines",
                     name="L(n)", hovertemplate="n=%{x}<br>L=%{y:.3f}<extra></extra>")
    fig1.update_layout(
    ...
    fig2 = go.Figure()
    fig2.add_scatter(x=sig_grid, y=L_sig, mode="lines",
                     name="L(σ)", hovertemplate="σ=%{x:.2f}<br>L=%{y:.3f}<extra></extra>")
    fig2.update_layout(
    ...
    fig3 = go.Figure()
    fig3.add_scatter(x=conf_grid, y=L_conf, mode="lines",
                     name="L(conf)", hovertemplate="신뢰도=%{x:.1f}%<br>L=%{y:.3f}<extra></extra>")
    fig3.update_layout(
    ...
    with st.expander("🧮 공식 요약", expanded=False):
        ...
        st.markdown(
            ...
            f"- z(α/2) = **{z:.3f}**, 길이 L = **{L:.3f}** (반길이 **{L/2:.3f}**)"
        )
