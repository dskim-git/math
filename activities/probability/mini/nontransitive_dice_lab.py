...
def pairwise_table(names: list[str], probs: dict[tuple[str, str], tuple[float, float, float]]) -> pd.DataFrame:
    ...
    for r in names:
        row = []
        for c in names:
            row.append(np.nan if r == c else probs[(r, c)][0])
        mat.append(row)
        ...
        for i in range(6):
            ...
            with rows[r][c]:
                v = st.number_input(
                    ...
                    min_value=1, max_value=6,  # ★ 1~6으로 제한
                    step=1, format="%d",
                    key=f"{label}_f{i+1}"
                )
                vals.append(int(v))
...
def render():
    ...
    # ★ 컨트롤을 한 줄에 배치
    c_trials, c_seed, c_btn = st.columns([4, 2, 1])
    with c_trials:
    ...
    with c_btn:
        run = st.button("🔁 실행", use_container_width=True, key="nd_run")
    ...
    for x in names:
        for y in names:
            if x == y:
                continue
            exact[(x, y)] = prob_single_gt(dice[x], dice[y]) if mode_key == "single" else prob_double_sum_gt(dice[x], dice[y])
