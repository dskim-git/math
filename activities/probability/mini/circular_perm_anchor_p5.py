...
<div id="wrap">
  <div class="ui">
    ...
    <div class="hstack">
      ...
      <label class="hstack"><input type="checkbox" id="showAllRot" />
        <span class="note">현재 배치의 **회전 동치**들을 희미하게 모두 보기</span>
      </label>
    </div>
  </div>
  ...
  <div class="card note">
    ...
    <ul>
      ...
      <li>안쪽 원(색이 진함): 회전 중복을 제거한 **정준형(canonical)**—<b>사람 1번</b>이 항상 위쪽 고정</li>
...
function resetSeating(doShuffle=false){
  seating = [];
  for(let i=1;i<=n;i++) seating.push(i);
  if(doShuffle) shuffle(seating);
}
