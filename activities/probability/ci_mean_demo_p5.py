...
def guess_numeric_cols(df: pd.DataFrame):
    ...
    first = num_cols[0]
    s = df[first].dropna()
    looks_class = (str(first).strip() in ("반", "학급", "class", "Class")
...
const DATA = %s;
new p5((p)=>{
  const W=980, left=60, right=30, top=20, rowH=%d;
...
<div style="
  ...
  <div style="display:flex; gap:16px; align-items:center; flex-wrap:wrap;">
    ...
    <div style="font-size: 16px;">
      선택한 신뢰도: <b>{int(conf_pct)}%</b>  &nbsp;|&nbsp;  기대: <b>약 {int(conf_pct)}개</b> 포함
    </div>
    <div style="opacity:.8;">
      (표본 평균 신뢰구간은 진짜 모평균을 약 {int(conf_pct)}% 확률로 포함하도록 설계됩니다)
    </div>
  </div>
</div>
        """,
