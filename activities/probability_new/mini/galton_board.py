...
_SHEET_NAME = "갈톤보드시뮬레이션"

_QUESTIONS = [
    {"type": "markdown", "text": "**📝 이 활동과 관련된 이항분포 문제 2개를 스스로 만들고 풀어보세요**"},
...
const PAD_TOP  = 36;
const PAD_SIDE = 28;
const BUCKET_AREA_H = 110;  // 버킷 영역 높이
...
class Ball {
  ...
  // ── 핵심 수정: col은 이미 목적지 열로 업데이트돼 있으므로
  //   fromCol = col - (현재 스텝 dir==='R' ? 1 : 0)
  _setNextTarget() {
    const L = getLayout();
    if (this.pathIdx === 0) {
      // 진입점 → 첫 못 (0,0)
