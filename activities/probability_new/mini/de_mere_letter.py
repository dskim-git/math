...
/* ── SECTION ── */
.sc{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:20px 22px;margin:12px 0;animation:fadeIn .5s ease}
.sc-title{font-size:16px;font-weight:700;color:#ffd54f;margin-bottom:14px;display:flex;align-items:center;gap:8px}
@keyframes fadeIn{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}

/* ── PROGRESS DOTS ── */
.pdots{display:flex;justify-content:center;gap:10px;margin-bottom:14px}
.pd{width:11px;height:11px;border-radius:50%;background:rgba(255,255,255,0.15);cursor:pointer;transition:all .3s;border:2px solid transparent}
.pd.active{background:#ffd54f;transform:scale(1.3);border-color:#ffd54f}
.pd.done{background:#4CAF50;border-color:#4CAF50}
...
.hl-red{color:#FF7043;font-weight:bold}
...
.pcard{background:rgba(255,255,255,0.05);border:2px solid rgba(255,255,255,0.1);border-radius:14px;padding:14px 26px;text-align:center;min-width:130px;transition:all .4s}
...
.wdot{width:14px;height:14px;border-radius:50%;background:rgba(255,255,255,0.12);border:2px solid rgba(255,255,255,0.2);transition:all .3s}
...
input[type=range]{width:170px;-webkit-appearance:none;height:6px;border-radius:3px;outline:none;cursor:pointer}
...
.prizec{border-radius:14px;padding:18px;text-align:center;position:relative;overflow:hidden}
...
<div class="pdots" id="pdots">
  <div class="pd active" onclick="go(0)" title="편지"></div>
  <div class="pd" onclick="go(1)" title="상황"></div>
  <div class="pd" onclick="go(2)" title="시뮬레이션"></div>
  <div class="pd" onclick="go(3)" title="파스칼의 풀이"></div>
  <div class="pd" onclick="go(4)" title="정리"></div>
</div>
...
<div id="s0" class="sc">
  ...
  <div id="nextBtn0" style="display:none;text-align:center;margin-top:12px">
    <button class="btn bg" onclick="go(1)">상황 파악하기 ▶</button>
  </div>
</div>

<!-- ═══════════════ SCENE 1 : 상황 ═══════════════ -->
<div id="s1" class="sc" style="display:none">
  ...
  <div style="margin-top:14px">
    ...
    <div class="guess-row">
      <input type="range" id="gslider" min="0" max="64" value="32" step="1"
        style="background:linear-gradient(90deg,#64B5F6,#FF8A65)"
        oninput="upGuess()">
      <div>
  ...
  <div class="nav">
    <button class="btn bp" onclick="go(0)">◀ 이전</button>
    <button class="btn bg" onclick="go(2)">시뮬레이션 해보기 ▶</button>
  </div>
</div>

<!-- ═══════════════ SCENE 2 : 시뮬레이션 ═══════════════ -->
<div id="s2" class="sc" style="display:none">
  ...
  <div class="nav">
    <button class="btn bp" onclick="go(1)">◀ 이전</button>
    <button class="btn bg" onclick="go(3)">파스칼의 답장 ▶</button>
  </div>
</div>

<!-- ═══════════════ SCENE 3 : 파스칼 풀이 ═══════════════ -->
<div id="s3" class="sc" style="display:none">
  <div class="sc-title">📜 파스칼의 답장</div>

  <div class="lw" style="margin-bottom:14px">
  ...
  <div class="nav">
    <button class="btn bp" onclick="go(2)">◀ 이전</button>
    <button class="btn bg" onclick="go(4)">핵심 정리 ▶</button>
  </div>
</div>

<!-- ═══════════════ SCENE 4 : 정리 ═══════════════ -->
<div id="s4" class="sc" style="display:none">
  ...
  <div class="nav">
    <button class="btn bp" onclick="go(3)">◀ 이전</button>
    <div></div>
  </div>
</div>
...
<script>
// ── SCENE NAV ──────────────────────────────────────────
const SS = ['s0','s1','s2','s3','s4'];
let cur = 0;
function go(n){
  SS.forEach((id,i)=>{
    const el=document.getElementById(id);
    if(i===n){el.style.display='block';}else{el.style.display='none';}
  });
...
const LETTER_SEGS=[
  ...
  {t:'그런데 ',d:40},
  {t:'A가 2번, B가 1번을 이긴 상황에서 ',d:35,c:'gold'},
...
function typeLtr(){
  if(li>=LETTER_SEGS.length){
    document.getElementById('ltxt').innerHTML+=
      '<br><br><span style="color:#ffd54f;font-size:12px;cursor:pointer" onclick="go(1)">→ 상황 파악하러 가기 ▶</span>';
    document.getElementById('nextBtn0').style.display='block';
    return;
  }
  const seg=LETTER_SEGS[li];
  ...
  if(ci<seg.t.length){
    ...
    else if(seg.c==='red') html=`<span class="hl-red">${ch}</span>`;
    else html=ch==='<'?'&lt;':ch==='>'?'&gt;':ch;
    if(cursor) cursor.insertAdjacentHTML('beforebegin',html);
    ci++;
    ltimer=setTimeout(typeLtr,seg.d);
  }else{li++;ci=0;ltimer=setTimeout(typeLtr,8);}
}
...
def render():
    ...
    with t1:
        components.html(_HTML_LETTER, height=560, scrolling=False)
    with t2:
        components.html(_HTML_GAME, height=560, scrolling=False)
    with t3:
        components.html(_HTML_SIM, height=700, scrolling=False)
    with t4:
        components.html(_HTML_PASCAL, height=980, scrolling=False)
    with t5:
        components.html(_HTML_SUMMARY, height=480, scrolling=False)
    st.markdown("---")
