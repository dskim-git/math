...
_QUESTIONS = [
    {"type": "markdown", "text": "**📝 이 활동과 관련된 탐구 질문 2개를 만들고 답해보세요**"},
    {"key": "문제1",        "label": "탐구 질문 1",     "type": "text_area",  "height": 80},
    {"key": "답1",          "label": "질문 1의 답",     "type": "text_area",  "height": 80},
    {"key": "문제2",        "label": "탐구 질문 2",     "type": "text_area",  "height": 80},
    {"key": "답2",          "label": "질문 2의 답",     "type": "text_area",  "height": 80},
...
② 홀수인 이항계수($\\binom{n}{k}$)만 색칠되며 시에르핀스키 삼각형이 나타납니다.  
③ **확대 배율**을 낮춰 더 많은 행을 보세요 — 패턴이 무한히 반복됨을 확인하세요.

> **핵심 원리 (뤼카 정리)**: 소수 $p$에 대해 $\\binom{n}{k} \\equiv \\prod_{i} \\binom{n_i}{k_i} \\pmod{p}$  
> 여기서 $n_i, k_i$는 $n, k$의 $p$진법 각 자리수입니다.  
> $k$의 어느 자리가 $n$의 대응 자리보다 크면 $\\binom{n}{k} \\equiv 0$이 됩니다.
""")
...
""")
    ...
    for i, q in enumerate(_QUIZ):
        with st.container():
            ...
            sel_key = f"pascal_quiz_sel_{i}"
            chk_key = f"pascal_quiz_chk_{i}"
