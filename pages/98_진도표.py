...
with _nav_col:
    ...
    if st.button("🏠 홈으로", key="_98_nav_home", use_container_width=True):
        st.switch_page("home.py")
    if st.session_state.get("_dev_mode", False):
        st.divider()
        st.caption("🔧 관리자 기능")
        if st.button("👥 회원관리", use_container_width=True, key="_98_nav_member"):
            st.switch_page("pages/97_회원관리.py")
        if st.button("📥 피드백 게시판", use_container_width=True, key="_98_nav_feedback"):
            st.session_state["_nav_to"] = "feedback_board"
            st.switch_page("home.py")
        if st.button("📊 방문자 통계", use_container_width=True, key="_98_nav_stats"):
